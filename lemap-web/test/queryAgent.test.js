...
const state = {
  fields: {
    'field:n': { enabled: true, visible: true, checked: true },
    'field:y': { enabled: true, visible: true, checked: false },
    'field:c1': { enabled: false, visible: true, checked: false },
    'field:c2': { enabled: false, visible: true, checked: false }
  }
};

test('query agent asks only currently actionable unresolved local groups', () => {
  const questions = buildUserQuestions({ graph, state, answeredGroupIds: new Set() });
...
test('user answer model contract maps natural language to known structural options only', () => {
  const question = buildUserQuestions({ graph, state, answeredGroupIds: new Set() })[0];
