...
test('irreversible material-like select is explored by disposable probes while live page stays untouched', async (t) => {
  ...
  const diagnostics = JSON.stringify({ yearId: year.id, onlineId: online.id, errors: result.errors, selectObservations }, null, 2);
