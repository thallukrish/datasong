...
import { redactRegisteredModelText } from '../semantic/modelPrivacy.js';

function arr(value) { return Array.isArray(value) ? value : []; }
function clean(value, max = 260) {
  const text = redactRegisteredModelText(String(value ?? '')).trim().replace(/\s+/g, ' ');
...
export function compactModelResult({ purpose = '', model = '', durationMs = 0, usage = null, finishReason = '', parsed = {} } = {}) {
  ...
  const result = {};
  for (const key of ['decision', 'confidence', 'localCompletion', 'selectedEntityId']) {
  ...
  if (arr(parsed?.questionIds).length) result.questionIds = arr(parsed.questionIds).slice(0, 6).map((item) => clean(item, 160));
  if (arr(parsed?.selectedFieldIds).length) result.selectedFieldIds = arr(parsed.selectedFieldIds).slice(0, 6).map((item) => clean(item, 160));
...
export function summarizeUserInteraction({ question = {}, interpretation = {} } = {}) {
  ...
  const base = {
    ...
    interpretation: isValue ? 'value interpreted' : 'choice interpreted'
  };
  if (question.answerKind === 'choice') {
    ...
    return {
      ...base,
      selected: arr(interpretation.selectedFieldIds).map(String).map((fieldId) => ({ fieldId: clean(fieldId, 160), label: clean(labels.get(fieldId) || '', 180) }))
    };
  }
...
export async function createRunLogger({ baseDir = path.join('data', 'query-runs'), goal = '' } = {}) {
  ...
  await write('run_start', { goalProvided: String(goal || '').trim().length > 0 });
