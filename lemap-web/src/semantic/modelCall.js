import { redactRegisteredModelText } from './modelPrivacy.js';
...
export async function callJsonModel({ client, model, systemPrompt, userPrompt }) {
  ...
  const safeSystemPrompt = redactRegisteredModelText(String(systemPrompt || ''));
  const safeUserPrompt = redactRegisteredModelText(String(userPrompt || ''));
  const purpose = purposeFromPrompt(safeUserPrompt);
  ...
  const raw = redactRegisteredModelText(response?.choices?.[0]?.message?.content || '{}');
  let parsed;
