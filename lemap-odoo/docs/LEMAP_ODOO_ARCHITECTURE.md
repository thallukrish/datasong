# LeMap Odoo Architecture

## Status

This document is the canonical architecture note for adding Odoo support to LeMap.
...
LeMap should not stop at an Odoo reference such as `_inherit = "mrp.production"` and treat Odoo as a permanent black box.

It should also not crawl all of Odoo up front.

Learning begins from the customer/project code and follows only the standard Odoo models, relationships, methods, views, actions, and workflows that are materially referenced or extended by that project.

---

## 2. Improvement over the current PopCommerce/Moqui persistence model
...
Framework-map enrichment and project-map enrichment are part of the same learning run.
...
The exact physical filenames may follow current LeMap persistence conventions during implementation, but the ownership boundary must remain framework versus project.
...
The generic LeMap engine remains responsible for generic program/data relationships and semantic learning.
...
into common deterministic evidence/topology that LeMap already consumes.

Higher semantic layers should reason in generic concepts such as:
...
rather than becoming Odoo-specific.
...
ACME EMS may reference or extend models such as:
...
This follows LeMap's existing boundary-aware philosophy: technical adjacency alone is not a reason to traverse an entire framework.
...
When a later Odoo 19 project references the same framework entities, LeMap reuses existing knowledge immediately and explores only missing or changed framework evidence.
...
The first project map represents the fictional ACME EMS Odoo 19 implementation.
...
These project-specific concepts should connect to standard Odoo entities through stable framework references rather than duplicate framework entities.
...
`lemap-odoo/` is the runnable Odoo profile/demo, not a fork of the LeMap engine.
...
lemap-odoo/
  README.md
  docs/
    LEMAP_ODOO_ARCHITECTURE.md
  config/
  scripts/
```

`demo_v2` remains the shared LeMap engine/reference implementation. `lemap-odoo` provides the Odoo-specific runner/configuration experience and uses ACME EMS as the first end-to-end target.

Persistent semantic maps remain in generic semantic-map storage, not as hard-coded knowledge inside the Odoo runner.
...
Splitting the existing PopCommerce semantic map into a reusable Moqui framework map and a PopCommerce delta is a later migration. The Odoo implementation should establish the generic layered-map mechanism so that migration becomes possible without introducing a Moqui-specific redesign.
