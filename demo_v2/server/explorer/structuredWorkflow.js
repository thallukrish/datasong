...
export const withStructuredWorkflow = (Base) => class StructuredWorkflowExplorer extends Base {
  ...
  wholeFlowPrompt(observation) {
    ...
    const contract = {
      ...
      arcUpdate: {
        ...
        workflowSteps: [{
          name: 'short business step name', description: 'what actually happens in this step and why it matters',
          entities: ['business entities participating in this step'], persistentObjects: ['records/documents persisted or read in this step'],
          effect: 'state change or visible result', sourcePath: 'best matching supplied source path, else empty'
        }],
        entityDetails: [{
          name: 'entity name', description: 'business meaning of this entity in this workflow',
        ...
        relationshipDetails: [{
          from: 'source entity or step', relation: 'business relationship/action', to: 'target entity or step',
    ...
    return [
      ...
      '- EXECUTABLE_FLOW.functionEvidence, when present, contains source bodies for functions on the admitted flow family. Treat those bodies as direct implementation evidence for conditions, data movement, calls, and state changes.',
      '- Do not infer behavior that is absent from functionEvidence; when functionEvidence is absent, rely only on the supplied structural flow and schemas.',
      ...
      '- Use only evidence in the supplied deterministic flow, structuralEvidence, functionEvidence, and entity schemas. Never invent persistence or behavior.',
  ...
  evidenceDepth(arc) {
    ...
    return {
      stages: stages.length, entities: entities.length, persistentObjects: persistent.length, relationships: relationships.length,
