...
export const withCallPathLookup = (Base) => class CallPathLookupExplorer extends Base {
  rankedPathById(id) {
  ...
  groupedPathForArc(arc) {
    if (!arc) return null;
    const top = typeof this.topology?.topCallPaths === 'function' ? this.topology.topCallPaths(500) : [];
