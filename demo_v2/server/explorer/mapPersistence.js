...
export const withMapPersistence = (Base) => class MapPersistenceExplorer extends Base {
  ...
  restorePersistedMapIfAvailable() {
    ...
    try {
      ...
      live.mapPersistence = { restored: true, savedAt: saved.savedAt || '', repoUrl: saved.repoUrl, commit: saved.commit, version: MAP_VERSION };
  ...
  closeCompletedArcs() {
    for (const arc of arr(this.state?.pass1Arcs)) {
      ...
      if ((flow.completed || (interpreted && noPendingBranches && Number(arc.progress || 0) >= 90)) && noPendingBranches) {
        ...
        arc.closureReason = flow.completed ? 'compressed flow fully interpreted' : 'high-confidence flow with no unresolved branches';
  ...
  persistSemanticMap() {
    ...
    try {
      this.closeCompletedArcs(); this.enrichTraceability();
      fs.mkdirSync(this.mapDirectory(), { recursive: true });
      const savedAt = new Date().toISOString();
      const semanticState = clone(this.state);
      semanticState.currentArtifact = null; semanticState.frontier = []; semanticState.executionStack = []; semanticState.stopRequested = false; semanticState.status = 'saved';
      fs.writeFileSync(this.mapFilePath(), JSON.stringify({ version: MAP_VERSION, repoUrl: this.state.repoUrl, commit: this.state.commit, savedAt, semanticState }, null, 2));
  ...
  emit() {
    ...
    if (this._stoppedByUser && this.state?.status === 'complete') this.state.lastMessage = 'Stopped by user. The learned map has been saved.';
    this.closeCompletedArcs(); this.enrichTraceability();
  ...
  applyDelta(parsed, observation) {
    ...
    this.closeCompletedArcs(); this.enrichTraceability(); this.persistSemanticMap();
    return result;
  }
