...
const explorer = new ProgressiveRepositoryExplorerV30({ topology, dataRoot, onState: (state) => broadcast(state) });
let running = false;
...
app.listen(port, () => {
  ...
  console.log('[DataSong v2] CALL-PATH PREPROCESSOR + XML adapters → boundary/containment classifier ─┴→ PASS 1 → PASS 2; detailed traces go to data/runs/*.jsonl');
});
