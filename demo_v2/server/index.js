...
const explorer = new ProgressiveRepositoryExplorerV34({ topology, dataRoot, onState: (state) => broadcast(state) });
let running = false;
...
app.listen(port, () => {
  ...
  console.log('[DataSong v2] CALL-PATH PREPROCESSOR + XML adapters → normalized flow-family compression → compact classifier → deterministic containment ─┴→ PASS 1 → PASS 2; detailed traces go to data/runs/*.jsonl');
});
