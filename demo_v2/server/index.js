...
const explorer = new ProgressiveRepositoryExplorerV20({
  topology,
  dataRoot,
...
app.listen(port, () => {
  ...
  console.log('[DataSong v2] PASS 1 compact mode prompts → semantic source-container alias normalization → Pass-1 arc/durable-thread ID normalization → typed artifact-operation normalization → detailed-current/signature-only-candidates → lazy XML/JMX hierarchy → ordered word-level search → alternate-keyword recovery → deterministic directory-choice recovery → multi-arc switching; detailed traces go to data/runs/*.jsonl');
});
