...
const explorer = new ProgressiveRepositoryExplorerV31({ topology, dataRoot, onState: (state) => broadcast(state) });
let running = false;
...
app.listen(port, () => {
  ...
  console.log('[DataSong v2] CALL-PATH PREPROCESSOR + XML adapters → boundary classifier → deterministic containment ─┴→ PASS 1 → PASS 2; detailed traces go to data/runs/*.jsonl');
});
