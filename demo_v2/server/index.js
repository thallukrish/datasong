...
const explorer = new ProgressiveRepositoryExplorerV42({ topology, dataRoot, onState: (state) => broadcast(state) });
let running = false;
...
app.listen(port, () => {
  ...
  console.log('[DataSong v2] Pass 2 interprets an entire compressed call-path family in one model call; only explicitly unresolved branches get a follow-up. Scout alone scans for missing directions. Discovery is disabled.');
});
