...
export class ProgressiveRepositoryExplorerV52 extends ProgressiveRepositoryExplorerV48 {
  emptyState() {
  ...
  reconciliationState() {
    if (!this.state.entityReconciliation || typeof this.state.entityReconciliation !== 'object') {
      this.state.entityReconciliation = {};
    }
  ...
  entityContextInArc(arc, entityName) {
    ...
    const steps = arr(arc?.workflowSteps)
      ...
      .slice(0, 3)
      .map((step) => ({
        name: clean(step?.name, 120),
        description: clean(step?.description, 220),
        effect: clean(step?.effect, 150)
      }));
    const relationships = arr(arc?.relationshipDetails)
      ...
      .slice(0, 3)
      .map((rel) => ({
        from: clean(rel?.from, 100), relation: clean(rel?.relation, 120), to: clean(rel?.to, 100),
        description: clean(rel?.description, 200)
      }));
  ...
  existingMappingsFor(entityName) {
    ...
    return this.businessArcs().flatMap((arc) => arr(arc?.entityRepresentations)
      ...
      .map((item) => ({
        physicalEntity: String(item.physicalEntity || ''), relation: String(item.relation || 'represented_by'),
  ...
  candidateSchemasFor(entityName) {
    ...
    // Concrete schema entities seen in the same workflow contexts are the strongest candidates.
    ...
    // Name/identifier similarity only proposes candidates; it never asserts a mapping.
  ...
  reconciliationPacket() {
    return {
      unresolved: this.unresolvedTargets(),
      descriptions: this.fieldDescriptionTargets()
    };
  }
  ...
  applyMappings(mappings, allowedByBusiness) {
    ...
    for (const mapping of arr(mappings)) {
      ...
      for (const physical of arr(mapping?.physicalEntities)) {
        ...
        const relation = ['represented_by','stored_in','identified_by','referenced_through'].includes(physical?.relation)
          ? physical.relation : 'represented_by';
        for (const arc of this.businessArcs()) {
  ...
  markAttempts(packet) {
    ...
    for (const target of packet.unresolved) {
      state.attemptsByEntity[key(target.name)] = {
    ...
    for (const target of packet.descriptions) {
      state.descriptionAttempts[key(target.name)] = {
        fingerprint: target.fingerprint,
        attemptedAt: timestamp,
  ...
  async reconcileEntityBatch(reason = 'batch') {
    ...
    let result;
    try {
      result = await this.callAndRecordAttempt({
      ...
      // Do not mark fingerprints on a failed call: the same evidence should be retried later.
      await this.appendRunLog({
        type: 'entity_batch_reconciliation_failed',
        explorationStep: this.state.step,
        timestamp: new Date().toISOString(),
        reason,
        error: error.message
  ...
  async runScout(candidates) {
    // Scout's next batch starts only after currently admitted arcs drain. This is
    // the natural end-of-batch boundary for reconciling cross-workflow evidence.
  ...
  async finalReconciliation() {
    const state = this.reconciliationState();
    let totalChanged = 0;
    // A mapping created in pass 1 can expose new evidence for another pending
    // entity. Permit a tiny fixed point, but never an open-ended token loop.
    for (let i = 0; i < 3; i += 1) {
