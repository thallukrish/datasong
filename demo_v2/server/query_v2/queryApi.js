...
export function registerQueryV2Api({ app, explorer, queryClient, queryModel, dataRoot, onLatestLog = () => {} }) {
  ...
  const append = (file, type, payload = {}) => {
    fs.appendFileSync(file, `${JSON.stringify({ type, timestamp:new Date().toISOString(), ...payload })}\n`, 'utf8');
  };
  ...
  app.post('/api/query-map-v2', async (req, res) => {
    ...
    try {
      ...
      if (!directory?.groups?.length) {
        return res.status(409).json({ error:'The entity directory is not ready yet. Run/finish LeMap learning or directory maintenance first.' });
      }

      console.log(`\n[lemap query-v2] ${question}`);
      console.log(`[lemap query-v2] confidence-ordered semantic DFS over ${directory.groups.length} clusters and ${entityCount} graph entities`);
      ...
      append(queryLog, 'query_v2_complete', {
        question,
        response,
        cumulativeUsage:response?.investigation?.usage || {}
      });
