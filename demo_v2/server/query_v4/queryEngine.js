...
export async function runSemanticBestFirstQueryV4({ question, client, model, graph, directory, workflows = [], enterpriseContext = {}, log = () => {} }) {
  ...
  if (!complete) {
    const assessment = assessPlanForLearning({ question, logicalRequest, coverage, connectivity, accepted, evidencedGraph:grounded, explorationSteps:stepRef.value });
    const { learningRequest, stepAssessment } = assessment;
    log('query_v4_needs_learning', { learningRequest, stepAssessment, cumulativeUsage:{...usage} });
    return { status:'needs_learning', answer:'', nextStep:'Targeted learning is required to establish the missing query evidence.', learningRequest, stepAssessment,
